...
- `announcements` テーブルは公開サイトのブラウザが anon key + RLS で直接読む(他の公開データはビルド時に service role で取得)。RLS(`published=true and 'end_user' = ANY(audiences)`)が可視範囲の実体。
- `article_bookmarks`(読者の匿名ブックマーク)は、公開サイトのブラウザが anon で初めて**書き込む**対象。ただし直接 insert/delete は許さない(anon の DELETE を許すと `where true` で全消しできてしまうため)。付け外しは SECURITY DEFINER の `toggle_bookmark()` RPC 経由のみで、渡された `client_token` の行だけを操作し公開記事以外は拒否する。件数は集計ビュー `article_bookmark_counts`(生トークンは晒さない)で読む。匿名ゆえトークン量産による水増しは可能(監査精度を要さないモチベ表示用途として許容。将来 reader アカウント化で厳密化する想定)。
...
- 通常記事の公開は同一著者につき `post_interval_days`(初期値10)日に1回。依頼記事(`commissioned_by` 非null)は対象外。
  ただし、admin(trusted フラグ有: `auth.uid() is null or is_admin()`)は投稿間隔ルールを無視して他ライターの下書きを即座に公開できる
...
- 検索と `Area` ナビは、ハンバーガーで開くナビゲーションドロワー(`NavDrawer.astro`、
  画面外の左パネル)に収めている。常設サイドバーは廃止し、本文は全ページ全幅の1カラム。
- ドロワーの地域リンク(`Area` ナビ)は `src/lib/sidebar.ts` の `getAreaLinks(db)` が
  ビルド中に1回だけ取得し、モジュールスコープの Promise でメモ化したものを全ページが
  参照する。ドロワーは `Base.astro` 経由で**全ページ**に出るコンポーネントなので、
  素直に書くと1ページ1クエリ(記事数百枚なら数百クエリ)になる。ページ側(`props`)から
  データを渡すのではなく、`NavDrawer.astro` が自分で `getAreaLinks()` を呼ぶ構成にして、
