...
describe('renderPreviewHtml', () => {
  it('matches renderBlocksToHtml for the same content', async () => {
    ...
    const expected = await renderBlocksToHtml({ type: 'doc', content }, 'https://img.test');
    expect(await renderPreviewHtml(editor, 'https://img.test')).toBe(expected);
    editor.destroy();
  });
});
