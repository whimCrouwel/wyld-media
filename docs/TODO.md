...
- [ ] 同じ設定画面で、現在の設定値がフォームに表示されない(空欄になる) — 取得: `admin/src/lib/admin.ts:110-119`(`fetchSettings`)、フォームへの反映: `admin/src/pages/settings.astro:67-69`
 ...
 [ ] (環境起因、同類)`supabase test db` で `supabase/tests/database/01_schema.test.sql` のテスト18(「settings has initial row with defaults」)が失敗する — ローカルDBの `settings` 行(id=1)が開発中に既定値から変更されているため。まっさらなDBでは再現しないはず(2026-07-31、依頼トークン「使用済み」表示修正の作業中に確認)。
...
- [ ] (環境起因、`02_rls`テスト11と同類)ルート `npm test` の `tests/content.test.ts`「splits featured...」が failed — featured の期待slug(`kaigan-seisou`/`kigyou-no-mori`)が返らない。ローカルDBの記事データがCMS上の手動テストで変わっているためとみられる(2026-08-21、見出しレベル拡張・画像トリムUI作業中に確認。コード変更とは無関係で、`renderBlocksToHtml` 系テストはパス)。seed し直せば解消するはず。
