...
│      CMS_URL                        │
├─────────────────────────────────────┤
│  ⑤  Google Search Console           │  ← サイトマップ再提出
│      新URLで提出 + 旧URL削除        │
└─────────────────────────────────────┘
```
...
**これを忘れると Google が新サイトを発見できず、検索流入がゼロのまま数週間放置される**(2026-08 に実際に起きた。旧ドメイン時代のサイトマップ登録が 404 になり、新 URL は誰も提出していなかった)。

https://search.google.com/search-console → 対象プロパティ(現状は `wyld-crd.org` ドメインプロパティ。サブドメイン全部をカバーするので新サブドメインでも追加登録は不要)→ 左メニュー「サイトマップ」:

1. **新 URL で提出**: `https://<NEW_SITE>/sitemap-index.xml`
2. **旧サイトのサイトマップ登録を削除**: 旧 URL の行 → クリックして詳細 → 右上 ⋮ → Remove sitemap
3. **主要ページのインデックス登録をリクエスト**: 上部の URL 検査バーにトップページ・主要記事の URL を入れ、「インデックス登録をリクエスト」(priority crawl queue に入る。1日あたり件数制限あり)

なお `/sitemap.xml`(WordPress 時代のパス)→ `/sitemap-index.xml` の 301 リダイレクトを `vercel.json` で張ってあるので、旧パスを記憶しているクローラー(Bing 等)も自動回復する。
...
✅ GSC:     新 URL の sitemap-index.xml が Success・旧登録は削除済み
```
